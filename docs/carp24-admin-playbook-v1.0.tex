...
\fancyhead[R]{\small Carp24 \& Supabase — Administrator Playbook (v1.1)}
\fancyfoot[C]{\thepage}
...
\midrule
v1.1 & 21.08.2026 & Wartungsmodus-Kapitel ergänzt (Schalten, Wirkung, Technik) \\
...
\subsection{\changedtitle{Wartungsmodus schalten} \changed{}}
Bei Wartungsarbeiten kannst du die gesamte Plattform auf eine Wartungsseite stellen:
\begin{enumerate}
    \item Admin-Dashboard → \textbf{Übersicht} → Karte \textbf{WARTUNGSMODUS}
    \item Optional: Nachricht an Besucher eingeben (z.B. „Wir sind bald wieder da!")
    \item Button \textbf{WARTUNGSMODUS AKTIVIEREN} — sofort aktiv
\end{enumerate}
\textbf{Wirkung:} Alle Besucher (nicht eingeloggt oder ohne Admin-Rolle) sehen \code{/wartung} (HTTP 503). \textbf{Admins} (Rolle ADMIN/MODERATOR) kommen überall durch — auch \code{/admin/*}. \code{/login}, \code{/impressum} und \code{/datenschutz} bleiben erreichbar (damit du dich als Admin einloggen kannst und Rechtstexte zugänglich bleiben). Zum Ausschalten: Button \textbf{WARTUNGSMODUS DEAKTIVIEREN} (gleiche Karte).
\textbf{Technik:} Der Status liegt in der Tabelle \code{app\_settings} (Key \code{maintenance}, Migration 0031) — überlebt Server-Neustarts. Ein globaler Astro-Middleware-Guard prüft jeden Request (5-Sekunden-Cache). Schalt-Aktionen landen im Audit-Log (\code{settings.maintenance}).
